...
traffacc <- traffacc %>% 
  ...
  mutate(MunLatin = ifelse(MunLatin == "Ǵorče Petrov", MunLatin = "Gjorče Petrov"))
```
...
# need to translate from cyrilic to latin alphabet
traffacc %>% 
  group_by(MunLatin) %>% 
  count %>% 
  rename(id=MunLatin) ->
...
# id's are in latin (LHS) and cyrilic (RHS)
...
ggplot(mkop.df3, aes(x = long, y = lat, group = group)) + 
  ...
  geom_polygon(colour="black", size=.1, aes(fill = n)) +
  ...
  labs(x=" ", y=" ") + 
  theme_bw() + 
  scale_fill_viridis('Број на незгоди', na.value="white" ) + 
  coord_map() + 
